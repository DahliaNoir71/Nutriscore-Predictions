...
def main():
    ...
    df_predictionss = make_predictions(model, scaler, df_test)
    display_predictions(df_predictionss)
    correct_predictions, total_predictions, accuracy_percentage = calculate_accuracy(df_predictionss)
    report = get_accuracy_report(correct_predictions, total_predictions, accuracy_percentage)
    plot_predictions_heatmap(df_predictionss, report)
